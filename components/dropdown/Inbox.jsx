...
function Inbox() {
  const { inbox, clearInobx, session, deleteNotifaction } = useStore();
  ...
  return (
    <div
      ...
      <ul name='inbox' className=' text-sm text-gray-700 z-[500] dark:text-gray-200'>
        {!session?.notificationsList?.filter((not) => not.type === "رسالة").length && (
          <li
          ...
          .map((message, index) => (
            <li
              ...
              <h2 className={`font-semibold text-right`}>
                <div onClick={() =>{
                if (session.isDoctor) {
                 deleteNotifaction(message._id,router, "doctors");
                }
                if (session.isUser) {
                  deleteNotifaction(message._id,router, "users");
                }
                if (session.isAdmin) {
                  deleteNotifaction(message._id,router, "admins");
                }                  
                } }>
                  <svg
