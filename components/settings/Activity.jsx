...
export default function Activity() {
  ...
  const normalButton = `inline-flex items-center justify-between w-full  gap-5 text-sm font-medium leading-5 rounded-full px-3 py-1 border border-slate-200 hover:border-slate-300 shadow-sm ${theme.nav} "text-slate-500" duration-150 ease-in-out`;
 const [dropdownOpen, setDropdownOpen] = useState(false);
  //store?.set("activity",[]);
  return (
    <div className={`grow overflow-y-auto shadow-lg h-[calc(100vh_-_190px)] w-full rounded-sm  relative "bg-white" "text-slate-800"`}>
      <table className="table-auto w-full divide-slate-200">
        {/* Table header */}
        <thead className={`text-xs sticky top-0 z-10 uppercase  text-center ${theme.nav} "text-slate-800" `}>
          <tr>
          ...
          {store
            ...
            .map((db, index) => (
              <tr key={uuidv4()} className={`${index & (1 === 1) && theme.main}`}>
                ...
                <td key={uuidv4()} className="text-center w-1/8 capitalize">
                  {db?.action === "supprimer" && <p className="capitalize text-center rounded-3xl px-1  bg-rose-10
                  text-rose-500">{db?.action}</p>}
