...
function page() {
  const { doctorInfo,session, updateNotification, handleSubmitDoctorUpdate, isLoading } = useStore();
 useEffect(() => useStore.setState({ doctorInfo: session }), [session]);
      ...
      <div id='personal-info' className='card rounded-md '>
        ...
        <form
          ...
          <div id='site' className='mt-4 flex gap-2 justify-between items-center grow'>
            ...
            <ToggleInput
              name='رسالة'
              checked={doctorInfo?.notifications?.includes("رسالة")}
              onChange={(e) => updateNotification(e, "رسالة", "doctorInfo")}
              type='text'
            />
          </div>
          ...
          <div id='site' className='flex gap-2 justify-between items-center grow'>
            ...
            <ToggleInput
              name='سؤال'
              checked={doctorInfo?.notifications?.includes("سؤال")}
              onChange={(e) => updateNotification(e, "سؤال", "doctorInfo")}
              type='text'
            />
          </div>
          <div className='font-semibold px-2 border-b-[1px] border-gray-300 dark:border-gray-700'></div>
          <div id='site' className='flex gap-2 justify-between items-center grow'>
            ...
            <ToggleInput
              name='موعد'
              checked={doctorInfo?.notifications?.includes("موعد")}
              onChange={(e) => updateNotification(e, "موعد", "doctorInfo")}
              type='text'
            />
          </div>
          <div className='font-semibold px-2 border-b-[1px] border-gray-300 dark:border-gray-700'></div>
          <div id='site' className='flex gap-2 justify-between items-center grow'>
            ...
            <ToggleInput
              name='تعليق'
              checked={doctorInfo?.notifications?.includes("تعليق")}
              onChange={(e) => updateNotification(e, "تعليق", "doctorInfo")}
              type='text'
            />
          </div>
          <div className='font-semibold px-2 border-b-[1px] border-gray-300 dark:border-gray-700'></div>
