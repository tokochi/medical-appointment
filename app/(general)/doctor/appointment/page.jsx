          ...
          <table className='text-sm whitespace-nowrap  md:text-base min-w-[600px] w-full'>
            ...
            <tbody>
              {appointments?.reverse()?.map((appoint, index) => (
                <tr
                  key={index}
                  className={`text-center text-sm border-b border-gray-300 dark:border-gray-600 hover:bg-sky-500/50 p-2`}>
                  <td className='text-center p-2'>
                    <div id='avatar' className='text-right flex font-roboto flex-col'>
                      <h1 className='text-sky-500'>{appoint?._doc.user?.name}</h1>
                      <h2 className=''>{appoint?._doc.user?.email}</h2>
                    </div>
                  </td>
                  <td className='p-2'>
