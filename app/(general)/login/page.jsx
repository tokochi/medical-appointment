  ...
  return (
    <div className='flex flex-col justify-center items-center  bg-gradient-to-r from-gray-100 to-gray-300 dark:from-slate-800 dark:to-slate-900'>
      <div className='flex gap-4 '>
        <button
          onClick={() => useStore.setState({ className: "login" })}
          className={`font-bold p-4 border-b-4 ${
            pathname === "login" ? "border-blue-400 text-blue-500" : "border-gray-400"
          }`}>
          تسجيل الدخول
        </button>
        <button
          onClick={() => useStore.setState({ className: "signup" })}
          className={`font-bold p-4 border-b-4 ${
            pathname === "signup" ? "border-blue-400 text-blue-500" : "border-gray-400"
          }`}>
          إنشاء حساب
        </button>
      </div>
