...
export async function PUT(req, { params }) {
    ...
    try {
        ...
        const response = await Post.updateOne({ _id: params?.id }, { $set: data })
        if (response.acknowledged === true && response.modifiedCount === 1) {
            return new Response('Post updated successfully', { status: 200 }); // OK
        } else {
            return new Response('Failed to update Post', { status: 500 }); // Internal Server Error
        }
    } catch (error) {
...
export async function DELETE(req, { params }) {
    ...
    try {
        ...
        const response = await Post.deleteOne({ _id: params?.id })
        if (response.acknowledged === true && response.deletedCount === 1) {
            return new Response('Post Deleted successfully', { status: 200 }); // OK
        } else {
            return new Response('Failed to Delete Post', { status: 500 }); // Internal Server Error
        }
    } catch (error) {
        return new Response(JSON.stringify(error), { status: 500 });
    }
